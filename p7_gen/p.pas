...
uses analex, syntax, symtable, io, options, errors, 
     ambito, stats, gencode, asm_pcode;
...
begin
   ...
   { Inicializamos variables de generacion de codigo }
   init_gencode(); { text, code, AsmBool, Emitting }
   ...
   { Ensamblamos con dos pasadas }
   if not (error) then
   begin
      writeln('   (II) Fichero con pseudoinstrucciones -> a.inf');
      writeln('   (II) Fichero de codigo temporal -> temp.$$$');
      assembler();
      emitter(); 
      writeln('   (II) Fichero ensamblado -> a.asm');
      writeln('   (II) Fichero de codigo definitivo -> a.out');
   end;

   { Finalizamos los ficheros de generacion de CI }
   finish_gencode();
