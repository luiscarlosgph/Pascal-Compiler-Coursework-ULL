   ...
   type 
      ...
      ObjectRecord = record
         ...
         case class of
            ...
            CL_FIELD: (FieldDispl : integer; FieldType : ptr);
            CL_PROCEDURE, CL_PROGRAM: (ProcLevel, ProcLable : integer; 
                           LastParam : ptr);
            CL_RECORD_TYPE: (RecordLength : integer; 
                             LastField : ptr;);
            CL_VALUE_PARAMETER, CL_VAR_PARAMETER, CL_VARIABLE: 
               (VarLevel, VarDispl : integer; VarType : ptr);
      end;
      BlockRecord = record
                       TempLength, MaxTemp : integer; { GenCode }
                       LastObject : ptr;            
                    end;
      ...
      var
         ...
         TypeBoolean : ptr; { Puntero al ObjectRecord del 
                              boolean }
         TypeInteger : ptr; { Puntero al Integer del 
                              StandardBlock }
         TypeError : ptr;

   { Funciones }
   procedure search(name : integer; LevelNo : integer; 
                    var found : boolean; var obj : ptr);
   procedure insert(name : integer; kind : class; var obj : ptr);
   procedure lookup(name : integer; var obj : ptr);
   procedure set_();
   procedure reset_();
   procedure StandardBlock();
   procedure KindError(obj : ptr);
...
implementation
   ...
   begin
      ...
      inc(stacknow[BlockLevel]); { Cuenta del numero de elem. 
                                   en pila }
      search(name, BlockLevel, usado, obj);
      if((usado) and (obj^.kind <> CL_UNDEFINED) and (obj^.name <> NO_NAME)) then
      begin
         { msg := 'Identificador [' + atrib^.ptr^.lexeme + '] ambiguo'; }
         msg := 'Identificador ambiguo'; 
    ...
    *        inserta -> si es true significa que si no lo
    *                   encuentras lo tienes que insertar
    *                   si es false, si no lo encuentras
    *                   no lo insertas
    *}
   procedure lookup(name : integer; var obj : ptr);
   var
   ...
   begin
      ...
      begin
         ...
         { GC }
         BlockTable[BlockLevel].TempLength := 0;
         BlockTable[BlockLevel].MaxTemp := 0;
         { Fin GC }
      end
      else
   ...
   {* Procedimiento KindError
    * Muestra un error indicando que la clase
    * de un identificador es erronea
    *}
   procedure KindError(obj : ptr);
   var
      msg : string;
   begin
      ...
      begin
         msg := 'Identificador de clase no valido'; 
         write_error(SEMANTIC, msg);
      end;
   end;
