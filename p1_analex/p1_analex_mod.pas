...
var
   ...
   begin
      while((i <= size) and ((buffer^[i] = chr(SPACE)) or (buffer^[i] = chr(TAB)) or (buffer^[i] = chr(CR)) or 
                             (buffer^[i] = chr(LF)))) do 
      begin
...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            inc(Index);
            writeln(line_num, '    ', col_num, '          <', TOKEN_STR[word(tok) + 1], ',', atrib^.ptr^.lexeme, '>   '); 
         end
         else if(tok = NUMERALX) then
         begin
            writeln(line_num, '    ', col_num, '          <', TOKEN_STR[word(tok) + 1], ',', atrib^.value, '>   ');
         end
         else
